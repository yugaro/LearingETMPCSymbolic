...
template <typename T>
vector<vector<vector<int>>> operation(vector<vector<vector<int>>> Q, Ref<RMatrix<T>> Qind, double alpha, Ref<RMatrix<T>> Lambda, Ref<RMatrix<T>> cov, double noises, Ref<RMatrix<T>> ZT, Ref<RMatrix<T>> Y, Ref<RMatrix<T>> b, vector<Ref<RMatrix<T>>> Xqlist, Ref<RMatrix<T>> Uq, double etax, double epsilon, Ref<RMatrix<T>> ellin, Ref<RMatrix<T>> y_mean, Ref<RMatrix<T>> y_std)
{
    ...
    for (int idq = 0; idq < Qind.rows(); idq++){
        ...
        for (int idu = 0; idu < Uq.rows(); idu++){
            ...
            stds = y_std * sqrt(kstarstar + noises - (kstar.transpose() * cov.inverse() * kstar)(0, 0));

            xvecnext_l = xvec + means - (epsilon * b.cwiseProduct(y_std) + beta.cwiseProduct(stds).cwiseProduct(y_std) + etaxv + ellin);
            xvecnext_u = xvec + means + (epsilon * b.cwiseProduct(y_std) + beta.cwiseProduct(stds).cwiseProduct(y_std) + etaxv + ellin);
