...
class safetyGame:
    def __init__(self, vehicle, gpmodels, likelihoods, cov, alpha, Lambdax, b, data_num, noise, etax, Xsafe, Uq, gamma_param):
        ...
        self.Lambdax = [Lambdax[i].to('cpu').detach().numpy() for i in range(3)]
        self.y = [gpmodels.train_targets[i].to(
        ...
        self.etax_v = np.array([etax, etax, etax])

        self.beta = np.array([self.set_beta(self.b[i], self.y[i], self.cov[i])
                              for i in range(3)])
        self.epsilon = np.array([self.set_epsilon(self.alpha[i], self.Lambdax[i])
                                 for i in range(3)])
        self.gamma = (np.sqrt([2]) * np.array(self.alpha) -
                      self.epsilon) / gamma_param
        self.cout = np.array([self.set_c(self.alpha[i], self.epsilon[i])
                              for i in range(3)])
        self.cin = np.array([self.set_c(self.alpha[i], self.epsilon[i] + self.gamma[i])
                             for i in range(3)])
        ...
        self.ellin_max = np.array(
            [self.ellin[:, i].max() for i in range(3)])

    def set_beta(self, b, y, cov):
        return np.sqrt(b ** 2 - y @ np.linalg.inv(cov + np.identity(self.data_num) * self.noise) @ y + self.data_num)

    def set_epsilon(self, alpha, Lambda):
        return np.sqrt(2 * (alpha**2) * (1 - np.exp(-0.5 * self.etax_v @ np.linalg.inv(Lambda) @ self.etax_v)))

    def set_c(self, alpha, epsilon):
        return np.sqrt(2 * np.log((2 * (alpha**2)) / (2 * (alpha**2) - (epsilon**2))))

    def min_max_check(self, x, xlist, dim):
        return torch.all(x + self.ellout[:, dim] <= xlist.max()) and torch.all(xlist.min() <= x - self.ellout[:, dim])
    ...
    def operation(self):
        ...
        X_range_min = np.array([X0.min(), X1.min(), X2.min()])
        X_range_max = np.array([X0.max(), X1.max(), X2.max()])

        Q = np.zeros([X0.shape[0], X1.shape[0], X2.shape[0]])
