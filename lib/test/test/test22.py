...
class safetyGame:
    def __init__(self, vehicle, gpmodels, likelihoods, cov, b, data_num, noise, etax, Xsafe, Uq, gamma_param):
        ...
        self.epsilon = torch.tensor([self.set_epsilon(self.alpha[i], self.Lambdax[i])
                                     for i in range(3)])
        self.gamma = torch.tensor([(1.41421356 * self.alpha[i] - self.epsilon[i]) / self.gamma_param[i] for i in range(3)])
    ...
    def operation(self):
        ...
        #             print(torch.sqrt(variances[0]), torch.sqrt(
        #                 variances[1]), torch.sqrt(variances[2]))
...
Xsafe = torch.tensor([[-1., 1.], [-1., 1.], [-1., 1.]])
