...
def getMap(markers, x, y, pos_reg):
    fig, ax = plt.subplots(1, 1)
    # ax.plot(markers[:, 0], markers[:, 1], 'ro', label="controlpoint")
    ax.plot(x, y, label="splprep")
    for i in range(len(x)):
        c = patches.Rectangle(
            xy=(x[i] - pos_reg, y[i] - pos_reg), width=pos_reg * 2, height=pos_reg * 2, fc='g', ec='r')
        ax.add_patch(c)
    ax.set_title("spline")
...
def train(z_train, y_train):
    ...
    for i in range(gp_updata_time):
        ...
        # print('aaaa')
        # print(gpmodels.models[0].covar_module.base_kernel.lengthscale)
        # print(gpmodels.models[1].covar_module.base_kernel.lengthscale)
        # print(gpmodels.models[2].covar_module.base_kernel.lengthscale)
        loss.backward()
        optimizer.step()
    ...
    # print('tesaset')
    # print(K_check(z_train[1:].double(), alpha[0].double(), Lambda[0].double()))
    # print(cov[0])
...
class safetyGame:
    ...
    def set_beta(self, b, y, cov):
        return torch.sqrt(b ** 2 - y @ torch.inverse(cov + torch.eye(self.Time) * self.noise) @ y + self.Time)
    ...
    def set_c(self, alpha, epsilon):
        return torch.sqrt(torch.log(((2 * (alpha**2)) / (2 * (alpha**2) - (epsilon**2)))**2))
    ...
    def operatioin(self, pos_reg, yaw_reg, V, Omega, L, b):
        ...
        print('ddddd')
        print(torch.exp(-0.5 * self.etax_v @ torch.inverse(self.Lambdax[0]) @ self.etax_v))
        print(torch.exp(-0.5 * self.etax_v @ torch.inverse(self.Lambdax[1]) @ self.etax_v))
        print(torch.exp(-0.5 * self.etax_v @ torch.inverse(self.Lambdax[2]) @ self.etax_v))
        print(torch.tensor(self.epsilon))
        print(torch.tensor(self.alpha))
        print(self.c[0])
        print(self.c[1])
        print(self.c[2])
        print(self.Lambdax[0])
        print(self.Lambdax[1])
        print(self.Lambdax[2])
        print(self.c[0] * self.Lambdax[0])
        print(torch.sqrt(self.c[0] * self.Lambdax[0]))
        ...
        for i in range(Qind.shape[0]):
            ...
            for j in range(V.shape[0]):
                for k in range(Omega.shape[0]):
                    ...
                    means = torch.tensor(
                        [predictions[l].mean for l in range(x_next_part.shape[0])])

                    x_next_real = self.vehicle.RK4(
                        X0[Qind[i, 0]], X1[Qind[i, 1]], X2[Qind[i, 2]], V[j], Omega[k])

                    variances = torch.tensor(
                        [predictions[l].variance for l in range(x_next_part.shape[0])])
                    ...
                    # print('a')
                    # print(torch.tensor(
                    #     [X0[Qind[i, 0]], X1[Qind[i, 1]], X2[Qind[i, 2]]]))
                    # print(x_next_real)
                    # print(x_next_part + means)
                    # print(variances)
                    # print(torch.tensor([self.beta]))
                    # print(torch.tensor([self.L * self.etax + self.b[l] * self.epsilon[l] + self.beta[l] *
                    #                     torch.sqrt(variances[l]) + self.etax for l in range(x_next_part.shape[0])]))
                    # print(xpre_lower)
                    # print(X_range_min)
                    # print(xpre_upper)
                    # print(X_range_max)
...
if __name__ == '__main__':
    # set params
    markers = np.array([[0, 0], [0.5, -1.2], [2, -2], [4, -1], [6, 1], [8, 2], [8, 5],
                        [6, 6], [4, 5.5], [2, 4.5], [1, 3], [0.5, 2], [0, 0]])
    marker_num = 50
    ts = 0.1
    noise = 0.000001
    v_max = 1
    ...
    getMap(markers, posx, posy, pos_reg)
