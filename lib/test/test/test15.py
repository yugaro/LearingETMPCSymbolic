...
class ETMPC:
    ...
    def cF(self, pg):
        c = [np.sqrt(2 * np.log((2 * (self.alpha[i]**2)) / (2 * (self.alpha[i]**2) - (pg[i]**2)))) for i in range(3)]
        return c
    ...
    def triggerValue(self, mpc):
        for i in reversed(range(self.horizon)):
            ...
            pg = psi.value
            trigger_values = np.concatenate([trigger_values, psi.value.reshape(1, -1)], axis=0)
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        for j in range(horizon):
            ...
            print(etmpc.triggerF(xstar_next, xe_next, trigger_values[j + 1]))
