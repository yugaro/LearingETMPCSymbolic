...
euler_lagrange = vertcat(
    # 1
    h1 * ddpos + h2 * ddtheta[0] * \
    cos(theta[0]) + h3 * ddtheta[1] * cos(theta[1]) - (h2 * dtheta[0] ** \
                                                       2 * sin(theta[0]) + h3 * dtheta[1]**2 * sin(theta[1]) + u),
    # 2
    h2 * cos(theta[0]) * ddpos + h4 * ddtheta[0] + \
    h5 * cos(theta[0] - theta[1]) * ddtheta[1] - (h7 * \
                                                  sin(theta[0]) - h5 * dtheta[1]**2 * sin(theta[0] - theta[1])),
    # 3
    h3 * cos(theta[1]) * ddpos + h5 * cos(theta[0] - \
                                          theta[1]) * ddtheta[0] + h6 * ddtheta[1] - \
