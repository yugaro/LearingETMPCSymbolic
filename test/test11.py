...
def train(z_train, y_train, gpudate_num):
    ...
    cov = [(gpmodels(*gpmodels.train_inputs)
            [i].covariance_matrix).to(device) for i in range(y_train.shape[1])]

    gpmodels.eval()
    likelihoods.eval()
    alpha = [torch.sqrt(gpmodels.models[i].covar_module.outputscale).to(device)
             for i in range(y_train.shape[1])]
    Lambdax = [(torch.diag(
        gpmodels.models[i].covar_module.base_kernel.lengthscale.reshape(-1)[:3]) ** 2).to(device) for i in range(y_train.shape[1])]
    Lambda = [(torch.diag(
        gpmodels.models[i].covar_module.base_kernel.lengthscale.reshape(-1)) ** 2).to(device) for i in range(y_train.shape[1])]
...
class safetyGame:
    def __init__(self, vehicle, gpmodels, likelihoods, cov, alpha, Lambdax, b, data_num, noise, etax, Xsafe, Uq, gamma_param):
        ...
        self.beta = torch.tensor([self.set_beta(b[i], gpmodels.train_targets[i].to(device), cov[i])
                                  for i in range(3)])
        ...
        while Qflag == 1:
            ...
            for i in range(Qind.shape[0]):
                ...
                for j in range(Uq.shape[0]):
                    ...
                    # print(self.beta)
                    # print(variances)
                    # print(self.beta[0] * torch.sqrt(variances[0]))
                    # print(self.beta[1] * torch.sqrt(variances[1]))
                    # print(self.beta[2] * torch.sqrt(variances[2]))
...
z_train, y_train = make_data(
    vehicle, xinit, data_num, v_max, omega_max)
