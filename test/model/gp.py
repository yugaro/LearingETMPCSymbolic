...
class GP:
    def __init__(self, z_train, y_train, noise):
        ...
        self.rbfk = RBF(length_scale=np.ones(z_train.shape[1]), length_scale_bounds=(1e-20, 1e20))
        self.whtk = WhiteKernel(noise_level=self.noise, noise_level_bounds=(1e-20, 1e20))
    ...
    def predict(self, z_test):
        ...
        means, stds = self.gpr.predict(z_test, return_std=True)
        return means, stds
