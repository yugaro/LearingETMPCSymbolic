...
class safetyGame:
    ...
    def set_beta(self, b, y, cov):
        return torch.sqrt(b ** 2 - y @ torch.inverse(cov + torch.eye(self.Time) * self.noise) @ y + self.Time / 100)
    ...
    def operatioin(self, pos_reg, yaw_reg, U, L, b):
        ...
        while Qflag == 1:
            ...
            for i in range(Qind.shape[0]):
                ...
                for j in range(U.shape[0]):
                    z_test = torch.tensor(
                        [X0[Qind[i, 0]], X1[Qind[i, 1]], X2[Qind[i, 2]], U[j, 0], U[j, 1]]).reshape(1, -1).double()
                    with torch.no_grad(), gpytorch.settings.fast_pred_var():
                        predictions = likelihoods(
                            *self.gpmodels(z_test, z_test, z_test))
                    x_next_part = self.vehicle.partF(
                        X0[Qind[i, 0]], X1[Qind[i, 1]], X2[Qind[i, 2]], U[j, 0], U[j, 1])
                    means = torch.tensor(
...
if __name__ == '__main__':
    ...
    U = torch.zeros(V.shape[0] * Omega.shape[0], 2)
    for i in range(V.shape[0]):
        for j in range(Omega.shape[0]):
            U[i * Omega.shape[0] + j, 0] = V[i]
            U[i * Omega.shape[0] + j, 1] = Omega[j]
    pos_reg = 0.5
    yaw_reg = 1
    ...
    safetygame.operatioin(pos_reg, yaw_reg, U, L, b)
